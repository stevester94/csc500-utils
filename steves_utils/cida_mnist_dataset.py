...
class CIDA_MNIST_DS(torch.utils.data.Dataset):
    ...
    def __init__(self, seed, domain_configs) -> None:
        """
        args:
            domain_configs: {
                "domain_index":int,
        ...
        for domain in domain_configs:
            ...
            ds = Rotated_MNIST_DS(seed, domain["min_rotation_degrees"], domain["max_rotation_degrees"], root="/mnt/wd500GB/CSC500/")

            for rando in self.rng.choice(len(ds), size=domain["num_examples_in_domain"], replace=False):
                example = ds[rando]
                self.data.append(
                    (example[0], example[1], domain["domain_index"], domain["source_domain"])
                )
